...
The use of ionizing radiation to treat malignant tissue has evidently shown to be effective \textcolor{red}{find a citation}. External beam therapy, brachytherapy and targeted radionuclide therapy utilizes radiation, where external beam therapy uses accelerated particles, brachytherapy uses a contained radioactive source, which is placed in proximity to tumor, and targeted radionuclide therapy uses the biochemical distribution in the body to deliver a radioactive payload, inside or attached to cancerous cells. \\

Ionizing radiation are particles with sufficient energy to cause ionizations along the particle track, thus separating an atom and one or more electrons. The consequence is one or more free electron(s) with the possibility to ionze other atoms, and a positive ion which can cause undesired reactions \textcolor{red}{find a citation}. DNA is a large molecule with two strands, bound in a double helix structure. Each strand is composed of sugar and phosphate groups, and nitrogeneos bases which bind the two strands together. These bases are called \textit{Adenine \& Guanin} and \textit{Cytosine \& Thyamine} (always bound pairwise). The bases are bound through weak hydrogen bonds and are easely exposed to strand breaks. The cell has an impressive repair mechanism, and unless both DNA strands are damaged, called a \textit{double stranded break}, most damages are repaired. Radiation-damages can be caused directly by the incoming particle, or indirectly via free radicals which are subject to other ionizations. Since the body contains a lot of water, ionization of water molecules giving H$^\bullet$, $e_\text{aq}^-$ or OH$^-$ are important damaging factors. Damages induced in the DNA can be lethal to the cell, and either cause apoptosis (cell death) or mutation which can cause cancer. In therapy, the goal is to cause cell death for cancerous cells, and choosing a type of particle which has a high linear energy transfer (LET) will induce double stranded breaks in the DNA if it passes nearby. Figure \ref{fig:DNA_damage} shows how beta-particles, alpha particles and auger electrons deposit energy on the scale of DNA.  \\

\begin{figure}
	\centering
	\includegraphics[width=0.7\textwidth]{Theory/DNA_LET.jpg} 
	\caption{Figure shows DNA .. \textcolor{red}{Cannot find citation anymore?} }
	\label{fig:DNA_damage}
\end{figure}


Particles interact differently dependent on their mass and charge. Figure \ref{fig:particles_Edeposition} shows an overview of how different particles deposit energy in a medium. The particles interact with the atomic electrons. In general, charged particles interact frequently with the atomic electrons due to the Coulomb force. For lighter particles like electrons and positrons, the energy transfer per collision can be up to half of its energy. For heavier particles such as protons and alpha-particles, the energy transfer is less, and can penetrate deeper into the tissue. The main cause of energy loss is due to Bethe-Block, which is an equation that decribes the stopping power if any particle heavier than the electron \textcolor{red}{Find a citation.}. The dose deposited in tissue is low until it has almost stopped, and deposits the majority of the energy in the Bragg peak. This makes hadrons suitable for treating small tumors with critical tissue surrounding it. Photons are neutral particles and rarely interacts. Photons has a long build-up where the secondary electrons increase the dose up to a certain depth. The depth of the maximum dose is dependent on the energy, and by using the energy which maximizes the dose over the tumor, the healthy tissue surrounding the tumor can be spared. However, the intensity of the photons decrease exponentially, so the dose behind the tumor will be exposed. 
%By rotating the beam around the patient, the tissue surrounding will be spared to a higher degree. Charged particles interacts more frequently due to the Coulomb force. Due to the equal mass of atomic electrons and electron beam, the energy is not deposited deep into the tissue. In general, the maximum energy transfer is given by the following equation



%where m is the atomic electron mass, M is the incoming particle mass, and E is the energy of the incoming particle. The heavier the particle and more energetic the particle, the less energy is transferred in one single collision. Electrons thus deposit up to half of its energy in one single collision, and thus have a short range. Hadrons, like protons, alpha particles loose little energy at high energies. The main cause of energy deposition is due to the Bethe-Block which is an equation that describes the stopping power of any particle heavier than the proton. The dose deposited in tissue is low until it has almost stopped, and deposits the majority of the energy in the Bragg peak. This makes hadrons suitable for treating small tumors with critical tissue surrounding it. External beam therapy has its limitations in 
...
External beam therapy utilizes the stoppingpower \textcolor{blue}{(is stoppingpower for photons as well?)} of particles like photons (X-rays), electrons and hadrons \textcolor{red}{find a citation}, and depending on the tissue, depth, shape of the tumor and energy of the particles, a dose plan can be made which maximizes the effect over the tumor and spare as much of the healthy tissue as possible. However, external beam therapy has its limitations. Firstly, it can only give dose to one single tumor, which is a limitation if metastasis are present. The exposure to healthy tissue is also present, allthough methods using hadrons can give a more specific dose distribution over the tumor, due to the Bragg peak. 
...
\subsection{General characteristics of medical isotopes used in therapy}

There are several physical and biological requirements for radiopharmaceuticals before they can be used clinically in therapy\textcolor{red}{from special curriculum. This is Yeong2014}. The physical properties do mostly concern radionuclides before they can be used clinically, like the physical half life, decay-type, radiation energy, decay products, the yield of production method and the purity of the production, which can be optimized by choosing an appropriate energy window. Important biological properties are the in-tissue range of the radiation,    concerning the cell-targeting molecule, 



and there are physical and biological properties. The physical properties mostly concern the radionuclide, like the physical half life, decay-type, radiation energy, decay products, production method. Important biological properties are concerning the cell-targeting molecule, such as uptake and retention in tumor, and the range of the radiation in the tissue. 

From a medical isotope production point of view, the physical 

\subsubsection*{\center{LET and stoppingpower}}

\subsubsection*{\center{Radioactive decay modes}}


\subsubsection*{\center{Half life}}

LET, stoppingpower, alpha vs. beta vs auger, decay, half life, decay products. Talk a little bit about tracers, and how that mainly affects the biological uptake. 

\subsubsection*{Production cross sections}
optimized prod, energy window. 


\subsection{$\mathbf{^{193m}}$Pt as a potential therapautic agent}

What is an isomer, why does it have a long half life. Parity, spin, decay. IT decay, auger electrons, dose, energy range. 

Platinum chemotherapy stuff. How we can use this isotope. Why do we believe that it will be effective? 
...
\section{The theory of an experimental cross section measurement}
Purity, foilsize, degradation. Want to find the optimum energy window. 


\subsection{Gamma-ray spectroscopy on HPGe-detectors}
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{Experiment/Illustration_beamOnTarget.png} 
	\caption{Illustration of the beam on target }
	\label{fig:beam_on_target}
\end{figure}
...
\begin{figure}%
    \centering \subfloat{{\includegraphics[width=8cm]{Results/Fe_56Co.png} }}%
    \subcaption{....}
    \quad
    \subfloat{{\includegraphics[width=8cm]{Results/Ni_61Cu.png} }}%
    \subfloat{{\includegraphics[width=8cm]{Results/Ni_56Co.png} }}%
    \quad
    \subfloat{{\includegraphics[width=8cm]{Results/Ni_58Co.png} }}%
    \quad
    \subfloat{{\includegraphics[width=8cm]{Results/Cu_62Zn.png} }}%
    \quad
    \subfloat{{\includegraphics[width=8cm]{Results/Cu_63Zn.png} }}%
    \quad
    \subfloat{{\includegraphics[width=8cm]{Results/Cu_65Zn.png} }}%
    \quad
...
\begin{figure}%
    \centering
    \subfloat{{\includegraphics[width=7.5cm]{Results/Ir_188Pt.png}}%
    \subcaption{...}
    \label{subfig:188Pt_plot}
    \quad
    \subfloat{{\includegraphics[width=7.5cm]{Results/Ir_189Pt.png} }}%
    \subcaption{...}
    \label{subfig:189Pt_plot}
    \subfloat{{\includegraphics[width=7.5cm]{Results/Ir_191Pt.png} }}%
    \subcaption{...}
    \label{subfig:191Pt_plot}
    \quad
    \subfloat{{\includegraphics[width=7.5cm]{Results/Ir_193mPt.png} }}%
    \subcaption{...}
...
\begin{figure}%
    ...
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ir_188Ir.png} }}
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ir_189Ir.png} }}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ir_190Ir.png} }}%
    \quad
    \subfloat[caption]{{\includegraphics[width=5cm]{Results/Ir_190m2Ir.png} }}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ir_192Ir.png} }}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ir_194Ir.png} }}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ir_194m2Ir.png} }}%
    \quad
...
\begin{figure}%
    \centering
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ni_52Mn.png}}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ni_54Mn.png} }}%
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ni_59Fe.png} }}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ni_60Cu.png} }}%
    \quad
    \subfloat[]{{\includegraphics[width=5cm]{Results/Ni_64Cu.png} }}%
    \quad
    \subfloat[caption]{{\includegraphics[width=5cm]{Results/Ni_60Co.png} }}%
    \quad
...
\centering
    ...
    %\begin{tabular}{|c|c|c|cc|cc|}
        ...
        \makecell[t]{$^{194m2}$Ir\\$\quad$(190+X)} & \makecell[t]{171 d} & \makecell[t]{\beta^-:100\%} & \makecell[t]{..} & \makecell[t]{..} & \makecell[t]{390.8} & \makecell[t]{35} \\ \hline
